...
def get_params():
    ...
    for row in range(1, sheet.max_row+1):
        ...
        if val:
            ...
            measure_v.append((val, sheet.cell(row=row, column=col+1).value, m_val))
            # print(measure_v)
            # input('for stop')
    for t in range(3):
        print(measure_v[t])
...
for each in measure_v:
    ...
    for k in range(4):
        item_name = k
        measure_val = each[2][k][1]
        mark = get_mark(gender, item_name, measure_val)
        each[2][k].append(mark)
...
for each in measure_v:
    row += 1
    # print(each[0])
    sheet.cell(row=row, column=1).value = each[0]
    sheet.cell(row=row, column=2).value = each[1]
    sheet.cell(row=row, column=3).value = each[2][0][0]
    sheet.cell(row=row, column=4).value = each[2][0][1]
    sheet.cell(row=row, column=5).value = each[2][0][2]
    sheet.cell(row=row, column=6).value = each[2][1][0]
    sheet.cell(row=row, column=7).value = each[2][1][1]
    sheet.cell(row=row, column=8).value = each[2][1][2]
    sheet.cell(row=row, column=9).value = each[2][2][0]
    sheet.cell(row=row, column=10).value = each[2][2][1]
    sheet.cell(row=row, column=11).value = each[2][2][2]
    sheet.cell(row=row, column=12).value = each[2][3][0]
    sheet.cell(row=row, column=13).value = each[2][3][1]
    sheet.cell(row=row, column=14).value = each[2][3][2]

wb.save('高考体育成绩表1.xlsx')
